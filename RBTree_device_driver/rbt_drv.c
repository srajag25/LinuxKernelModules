...
static char *user_name = "SpacemanSpiff";
...
struct rbtdevice_node *rb_search(struct rb_root *root, int key)
	...
	printk("Entering Search method...\n");
        struct rbtdevice *node = root->rbtdevice; 
...
void rb_insert(struct rb_root *root, struct rbtdevice_node *new)
	...
	//printk("Inserting node...");
        struct rbtdevice **link = &root->rbtdevice, *parent;
...
void inorder_deletion(struct rbtdevice *root)
{   
    if ( root!= NULL)
...
{	
	struct rbtdevice *node =  (rbtdevice_devp->root).rbtdevice; 
